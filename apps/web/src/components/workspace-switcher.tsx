 ...
 *
 * Mirrors the ProjectPicker's look. The active org is whatever's stored in
 * localStorage, falling back to the first membership (which matches the
 * server's default — earliest joined). Selecting a different workspace writes
 * the choice and does a full reload: the active-org header changes, so every
 * org-scoped query (and the locale + vertical providers) re-resolves cleanly.
 *
 * Single-membership users see just the workspace name — no dropdown.
 */
export function WorkspaceSwitcher() {
  ...
  const current =
    list.find((o) => o.orgId === stored) ?? list[0] ?? null;
  ...
  // Single workspace (or none): static label, no switcher affordance.
  if (!multi) {
    return (
      <div className="rounded-lg border border-ink-800 bg-ink-800/60 px-3.5 py-2.5">
        {label}
      </div>
    );
  }
